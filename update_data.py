...
from regime_model_v2 import classify as classify_regime
...
def calc(rows, breadth_info, supply_info=None):
    ...
    result = {
      ...
      'diagnostic':{'usable_points':len(vals),'rsi_ready':len(vals)>=15,'ma20_ready':len(vals)>=21,'ma60_ready':len(vals)>=61,'weekly_ready':len(vals)>=260},
    ...
    # V2: weekly direction + daily timing. Keep legacy score for compatibility.
    try:
        decision = classify_regime(rows)
        result.update(decision)
    except Exception as e:
        result['signal_state'] = 'UNKNOWN'
        result['signal'] = 'INSUFFICIENT'
        result['signal_reason'] = 'V2 classification error: ' + str(e)
    return result
...
for code in codes:
    try:
        ...
        # Market breadth / supply are enrichment layers. A temporary source/API
        # failure must NOT discard the stock price/regime calculation.
        try:
            breadth_info=fetch_breadth_and_nikkei(rows[0]['date'])
        except Exception as e:
            breadth_info={'date':rows[0]['date'],'breadth':{'6d':None,'10d':None,'15d':None,'25d':None},
                           'nikkei_value':None,'nikkei_change':None,
                           'source_url':'https://tofuhardboiled.com/updownratio/','error':str(e)}
        try:
            supply_info=fetch_supply(code,rows[0]['date'], info.get('name', code))
        except Exception as e:
            supply_info={'date':rows[0]['date'],'error':str(e),'api_partial':True,
                         'source_url':'https://api.irbank.net/v1/screening'}
